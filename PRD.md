...
- `MEMORY.md` (Core Memory): 로그 전체가 아닌, 드리밍 프로세스가 DB의 대화 원시 로그를 분석하고 도출해낸 **"핵심 요약본(완료된 작업, 장기간 유지할 정보)"**만을 텍스트로 깔끔하게 정리하여 보존.
- **시맨틱 메모리 (RAG):** 단순 텍스트 파일을 넘어 벡터 DB 중심의 검색 증강 생성을 활용해 방대한 양의 과거 이력을 자동 색인하고 문맥에 맞춰 쿼리.
- **드리밍(Dreaming) 및 기억 통합:** 1일 1회 실행을 원칙으로 하며 "마지막 입력 시점에서 2시간 초과 대기" 및 "지정된 심야 시간(예: 03:00) 통과" 조건을 동시 만족 시 동작. 단기 내역을 요약하여 페르소나(`USER.md`) 및 `MEMORY.md`를 자율 병합하며, 실행 전 반드시 `.bak` 백업 파일을 생성해 오염(Hallucination) 시 빠른 수동 롤백을 보장.
...
- **상태 및 데이터 저장 원칙**:
  - **전역 상태 (Global State)**: `AGENT.md`, `USER.md` 문서류는 기본 읽기 전용으로 취급하되, 특별히 지정된 드리밍(Dreaming) 요약 프로세스 등 통제된 권한 안에서만 제한적으로 병합.
