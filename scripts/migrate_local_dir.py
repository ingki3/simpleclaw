"""BIZ-133/BIZ-302 — `.agent/` (저장소 안) → base folder(기본: `~/.simpleclaw-agent/default`) 이전 스크립트.
...
base folder(기본 ``~/.simpleclaw-agent/default``)로 옮긴다. 실행 후에는 git 작업과 dreaming 런타임 쓰기가
물리적으로 다른 디렉터리를 보게 되어 race window 자체가 사라진다.

실행 후 동작
------------
- 기본 대상(``~/.simpleclaw-agent/default``)에 라이브 파일이 위치하고 dreaming/agent 가 그 위치를 사용.
...
def migrate(
    ...
    *,
    ...
    """`.agent/` 라이브 파일·디렉터리를 base folder(기본 ``~/.simpleclaw-agent/default``)로 이전한다.
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--target",
        type=Path,
        default=Path("~/.simpleclaw-agent/default").expanduser(),
        help=(
            "이전 대상 디렉터리 (기본: ~/.simpleclaw-agent/default — persona.local_dir 기본값과 일치)"
        ),
    )
    parser.add_argument(
        "--dry-run",
