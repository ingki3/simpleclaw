...
#### 실행 timeout (`settings.timeout`)

레시피 단위로 스텝별 최대 실행 시간(초)을 지정할 수 있습니다 (BIZ-423).

```yaml
settings:
  timeout: 180   # 각 command 스텝의 최대 실행 시간(초). 기본 60
```

- 수동 슬래시 명령(`/recipe-name`)과 Cron 예약 실행의 v1 `steps:` 경로 모두에 적용됩니다.
- 누락되거나 비정상 값(음수, 0, 문자열, bool)이면 기본 60초로 폴백하며, 경고 로그만 남기고
  레시피 파싱 자체는 실패시키지 않습니다.
