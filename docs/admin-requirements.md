...
* **스킬·레시피 활성/비활성 메타 키 (BIZ-47 후속, → BIZ-41)** — Admin UI는 스킬·레시피 단위로 즉시 토글 가능한 활성 플래그를 노출하지만, 현행 `skills.yaml`/`recipes.yaml` 스키마에는 해당 키가 없다. 후속 이슈에서 다음을 신설해야 한다.
  * `skills.disabled: list[str]` (또는 `skills.entries[].enabled: bool`) + `/admin/v1/skills/{id}` PATCH 핸들러.
  * `recipes.disabled: list[str]` + `/admin/v1/recipes/{id}` PATCH 핸들러.
  * 스킬 실행 로그 조회 엔드포인트 `GET /admin/v1/skills/{id}/runs?limit=N` — 현재 BIZ-25 trace 로그 저장소를 재사용 가능.
