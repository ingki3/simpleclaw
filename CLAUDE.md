...
## 문제 해결 우선순위
- **스킬에서 문제가 생겼을 때는 스킬 개선을 통해 해결하는 방법을 최우선한다.**
- 예를 들어, 스킬 결과에 사실 검증이 필요하면 orchestrator/tool-loop 같은 상위 시스템에서 결과를 사후 재평가·차단하기보다 해당 스킬의 데이터 수집, 검증, 출처 표시, 오류/무결과 처리 로직을 개선한다.
- 공통 런타임 가드는 보안·프로세스 격리·명확한 실행 실패처럼 도메인 스킬 내부에서 해결할 수 없는 cross-cutting 문제에 한해 도입한다.

## 시스템 프롬프트 관리 규칙
- 시스템에서 필요한 프롬프트는 코드에 하드코딩하지 않고 YAML 파일로 별도 관리한다.
- 저장 위치는 `prompts/system/` 아래로 통일한다.
- 프롬프트 파일에는 용도, 적용 범위, 변경 사유를 YAML 필드 또는 주석으로 남겨 후속 작업자가 런타임 영향 범위를 추적할 수 있게 한다.
