"""BIZ-639 — connected actual-provider no-send validator의 offline 회귀."""
...
async def _draft_response(_request) -> LLMResponse:
    return LLMResponse(
        text=json.dumps(
            {
                "content": "The activation gate is READY.",
                "cited_paths": ["data.category"],
                "limitation_paths": [],
            }
        )
    )
...
@pytest.mark.asyncio
async def test_connected_probe_measures_configured_sink_deltas() -> None:
    ...
    assert result["citations"] == ["data.category"]
