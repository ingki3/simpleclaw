...
설계 결정 — Markdown 이 source of truth:
    ...
    - :mod:`~simpleclaw.study.topic_registry` — ``topics.yaml`` ↔ ``StudyTopic``
      영속화와 진화형 topic 생애주기(candidate/active/pinned/cooling/archived).
...
source 계획/수집 계층(매일 학습할 topic 의 source 를 계획·수집·relevance 필터):
    ...
    - :mod:`~simpleclaw.study.interest_signals` — Dreaming 산출물/대화/사용자
      ...
      가중치로만 반영한다.
    - :mod:`~simpleclaw.study.scorer` — 관심도/반복/신선도/세상 중요도/최근성을
      하나의 점수로 합성하는 공식과 헬퍼. topic 진화의 상태 전이 판정에 쓰인다.
...
설계 결정 — registry 두 갈래:
    영속화 레지스트리 :class:`~simpleclaw.study.topic_registry.TopicRegistry` 는
    ``topics.yaml`` 의 source of truth 이고, 진화 레지스트리
    :class:`~simpleclaw.study.topic_registry.TopicEvolutionRegistry` 는 신호 기반
    생애주기를 in-memory 로 다룬다. 진화 계층의 관심 신호는 :class:`TopicSignal`
    로, Dreaming seed 신호 :class:`InterestSignal` 과 이름을 분리해 공존한다.
