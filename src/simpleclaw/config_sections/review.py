"""Review 서브시스템(config.yaml ``review`` 섹션) 설정 로더 (BIZ-440, BIZ-441).

subagent review ledger 와 verification evidence ledger 의 저장 경로/보존 기간을
로드한다. 다른 config loader 와 같은 원칙 — 파일이 없거나 파싱에 실패해도
항상 안전한 기본값으로 동작한다.
"""
...
# ledger 기본값. 경로는 다른 운영 데이터(conversations.db, suggestions JSONL 등)와
# 같은 런타임 루트 아래에 둔다. retention_days 는 완료(terminal) record 의 보존
# 기간 — 미완료 record 는 기간과 무관하게 보존된다 (각 ledger 의
# ``_apply_retention`` 참고).
...
def load_review_config(config_path: str | Path) -> dict:
    ...
    result: dict = {}
    for section, defaults in _REVIEW_DEFAULTS.items():
        ledger_raw = raw.get(section, {})
