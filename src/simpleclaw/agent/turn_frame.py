"""[Deprecated fallback] Follow-up/축약 사용자 발화의 결정적 맥락 정규화(TurnFrame).

BIZ-426 — primary turn 정규화는 이제 LLM 기반
``simpleclaw.agent.turn_analysis`` 가 수행한다. 이 모듈은 turn analysis
비활성(``agent.turn_analysis.enabled: false``) 또는 LLM provider 장애 시의
결정적 fallback 으로만 남아 있다. keyword cue(_FOLLOWUP_CUES 등)를 확장해
라우팅 문제를 풀지 말 것 — 안정화 후 별도 cleanup 이슈에서 제거될 수 있다.
...
설계 결정:
- LLM 을 쓰지 않는 cheap heuristic 만 사용한다 — fallback 경로는 provider
  장애 중에도 동작해야 하므로 레이턴시/비용을 더하지 않아야 한다.
