...
class TelegramBot:
    ...
    async def handle_message(
        ...
        *,
        ...
        BIZ-336: ``attachments`` 는 인증 이후에만 전달된다. 기존 텍스트-only
        handler 와의 호환을 위해 콜백 시그니처가 kwarg 를 받지 않으면 생략한다.
        """
        ...
        if self._message_handler:
            try:
                ...
                if kwargs:
                    kwargs = self._filter_handler_kwargs(kwargs)
    ...
    def _filter_handler_kwargs(self, kwargs: dict) -> dict:
        """message_handler 가 받을 수 있는 kwarg 만 남긴다.

        오래된 테스트/채널 handler 는 ``(text, user_id, chat_id)`` 만 받는다.
        사진이 없는 텍스트-only 경로는 그대로 3인자 호출하고, 사진/스트리밍 경로에서만
        이 필터를 적용해 새 기능과 기존 콜백을 함께 살린다.
        """
        if self._message_handler is None:
            return {}
        try:
            sig = inspect.signature(self._message_handler)
    ...
    @staticmethod
    def _message_text_with_attachment_default(message) -> str:
        """텍스트/캡션이 없는 이미지 메시지에 안전한 기본 프롬프트를 부여한다."""
        text = getattr(message, "text", None) or getattr(message, "caption", None) or ""
        if text.strip():
            return text
        if getattr(message, "photo", None) or getattr(message, "document", None):
            return "첨부한 이미지를 설명해 주세요."
        return ""

    @staticmethod
    def _image_file_from_message(message) -> tuple[str, str, str | None] | None:
        """Telegram message 에서 이미지 file_id, MIME type, filename 을 추출한다."""
        photos = list(getattr(message, "photo", None) or [])
        if photos:
            photo = photos[-1]
            return getattr(photo, "file_id", ""), "image/jpeg", None
    ...
    async def _download_image_attachment(
        ...
        """인증된 Telegram 이미지 첨부를 bytes 로 다운로드한다.

        non-image document 는 무시한다. 다운로드 실패는 메시지 처리 자체를 막지 않도록
        warning 으로 남기고 텍스트 경로로 폴백한다.
        """
        image_info = self._image_file_from_message(message)
        if image_info is None:
            return []
        file_id, mime_type, filename = image_info
        if not file_id:
            return []
        try:
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            async def _on_message(update, context):
                if update.message:
                    message_text = self._message_text_with_attachment_default(update.message)
                    if not message_text:
                        return
                    ...
                    if self.is_authorized(user_id, chat_id):
                        attachments = await self._download_image_attachment(
