...
- Stage A 만 끝내고 본인이 Stage B 를 이어서 진행해도 무방하다 — 단, 종료 조건을 동일하게 보장한 뒤 다음 단계로 진입.
...
**핸드오프**
- 자동 검증 대상이면 Review Agent / 운영자가 `in_review` 큐에서 픽업한다 (Stage D).
- 검증 불필요(문서·내부 도구·자명한 변경)면 본 단계에서 곧바로 `done` 전환 후 머지까지 진행할 수 있으나, **"검증 불필요" 판단이 모호하면 사용자에게 확인** (§2.1). 임의로 리뷰를 건너뛰지 말 것.
...
1. **직전 run 확인** — `multica issue runs <id> --output json` + `multica issue comment list <id> --output json` 으로 직전 실행과 코멘트를 먼저 본다.
2. **직전 코멘트가 본인이고 상태가 입력 대기**면 doublepost 금지 — "재확인 — 변화 없음" 한 줄만 박제하거나, 이미 박제된 운영자 액션 게이트가 풀리지 않았다면 코멘트 생략.
3. **타 에이전트가 동일 회귀를 반복 보고 중**이면 추가 코멘트 금지. 보드 노이즈만 키운다.
4. **동일 운영자 액션 게이트에 다수 형제 sub-issue(>5)가 모두 in_review 면 부모 이슈에 사이클당 1회만 박제** — 개별 sub-issue 중복 게시 금지.
