...
- [x] **BIZ-368: MCP tools capability gate 추가** — prompt-only/resource-only MCP 서버를 연결 실패로 오인하지 않도록 `MCPManager._connect_server()`가 `initialize()` 결과의 tools capability를 확인한 뒤에만 `list_tools()`를 호출. tools capability 없음은 connected server + tools 0개 로그로 기록하고, initialize 실패/list_tools 실패/stdio 연결 실패 메시지를 구분. list_tools 실패는 해당 서버만 스킵하며 다음 서버 연결 계속. 단위 테스트 3개 추가(`test_mcp_client.py`: prompt-only list_tools 미호출, tools capability 기존 등록 유지, list_tools 실패 격리). (2026-06-13)
