...
- [x] **BIZ-60: Admin UI · Lighthouse CI working-directory 수정** — `treosh/lighthouse-ci-action@v12`이 잡 `defaults.run.working-directory`를 무시해 리포 루트에서 `npm`을 찾다 `ENOENT package.json`으로 collect가 실패하던 문제(BIZ-55 게이트 미작동)를 해소. 액션 의존을 제거하고 `working-directory: web/admin`이 적용되는 일반 `run:` 스텝(`npm install -g @lhci/cli@0.14.x` → `lhci autorun --config=./lighthouserc.json`)으로 대체. 표준출력을 `.lighthouseci-output.txt`에 저장 후 `actions/upload-artifact`로 LHR과 함께 보존, `actions/github-script`로 임시 공개 스토리지 보고서 URL을 PR 코멘트로 자동 게시. 워크플로 권한에 `issues:write` / `pull-requests:write` 추가 (2026-05-03)
