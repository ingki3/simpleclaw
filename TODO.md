...
- [x] **BIZ-336: Telegram 이미지 첨부 멀티모달 처리 구현** — Telegram photo/document image 첨부를 인증 후 다운로드해 provider-neutral attachment로 라우터/Gemini까지 전달. Gemini는 inline image data(`types.Part.from_bytes`)로 변환하고, 텍스트-only handler 호환과 streaming/non-streaming 경로를 유지. 단위 테스트와 ruff 통과 (2026-06-03)
