...
| **시맨틱 메모리 (RAG)** | `sentence-transformers` 다국어 임베딩으로 메시지를 벡터 색인하고, 매 요청마다 의미상 가까운 과거 대화를 회상하여 시스템 프롬프트에 주입합니다. ReAct 루프 중에는 내장 `search_memory(query, top_k?)` 도구로 필요한 기억을 온디맨드 검색할 수 있습니다. SQLite + numpy만으로 동작 (외부 벡터 DB 불요). |
| **LLM 드리밍** | 심야에 LLM이 대화를 분석하여 기억 요약 → `MEMORY.md`, 사용자 인사이트 → `USER.md`에 각각 기록합니다. `AGENT.md`는 드리밍으로 수정되지 않습니다. 실행 전 `.bak` 백업을 생성합니다. 드리밍 모델은 `daemon.dreaming.model`로 설정 가능합니다. 프롬프트(system/user)는 `prompts/dreaming/{name}.yaml` 에서 SoT 로 관리됩니다 — 편집은 직접 수정 + PR. |
