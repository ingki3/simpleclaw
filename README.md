...
| **보안 실행 환경** | 위험 명령 감지(35개+ 패턴), subprocess 시크릿 스트리핑, 프로세스 그룹 격리로 안전한 명령 실행을 보장합니다. |
| **멀티턴 도구 실행** | LLM이 여러 도구를 순차적으로 호출하여 복잡한 작업을 자율적으로 처리합니다 (최대 5회 반복). |
...
사용자가 메시지를 보내면 에이전트는 다음 **멀티턴 도구 실행 루프**를 수행합니다:
...
[1] 스킬 판단 -------- "이 질문에 스킬이 필요한가?"
  |                     LLM이 사용 가능한 스킬 목록 + 이전 도구 결과를 읽고 판단
  |--- 불필요 ----------> [3]으로 이동
  |--- 필요 ------------> [2]로 이동
  |
[2] 보안 검사 + 스킬 실행
  |   (a) 위험 명령 감지 -- rm -rf, git push --force 등 35개+ 패턴 차단
  |   (b) 시크릿 스트리핑 - subprocess 환경에서 API 키/토큰 제거
  |   (c) 프로세스 격리 --- os.setsid로 프로세스 그룹 격리, timeout 시 그룹 kill
  |   (d) 스킬 실행 ------ 결과 수집 (메일, 주가, 검색 결과 등)
  |   |
  |   v
  |  결과를 컨텍스트에 추가 → [1]로 복귀 (최대 N회 반복)
  |
[3] 응답 생성
...
> 단일 도구 호출만 필요한 경우 루프는 1회만 실행되어 기존과 동일하게 동작합니다.
...
src/simpleclaw/
  ...
  agent.py        # 전체를 하나로 묶는 중앙 오케스트레이터 (멀티턴 루프)
  config.py       # 통합 설정 로더
```
...
agent:
  ...
  max_tool_iterations: 5               # 멀티턴 도구 호출 최대 횟수
...
7. **멀티턴 자율 실행** -- LLM이 필요한 만큼 도구를 반복 호출하되, 반복 횟수 제한으로 안전성 확보
